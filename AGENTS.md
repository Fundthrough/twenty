...
### 2026-07-27 — the "Cloudflare 502" postmortem + fail-loud push workflow
- **Root cause of ALL push 502s (Thu night + Mon): expired OUTREACH_ACCESS_TOKEN (2h), NOT platform errors.** outreach-push returned its "token expired" JSON with HTTP 502 → **Cloudflare replaces origin 5xx bodies with its own error page** → looked exactly like origin_bad_gateway. Rule: logic functions must never return 5xx for app-level errors; use 200+{error} (4xx passes through, 5xx gets masked).
- **Token refresh cadence SOLVED**: launchd agent `com.fundthrough.outreach-refresh` (90 min, ~/Library/LaunchAgents) runs standalone `~/.outreach-refresh.mjs` (no repo dependency) — refreshes rotating token pair + pushes app variable. Log: ~/.outreach-refresh.log.
- **Fail-loud workflow pattern**: HTTP step "Expected Response Body" sample JSON teaches the variable picker the response schema → downstream Code step binds response.prospectId and THROWS when empty → run shows red Failed with admin-contact message in the run panel (verified both paths: run #5 failed loud, run #8 completed green). HTTP steps NEVER fail on non-2xx by themselves (cloud swallows status).
- **Parallel-session safety**: main checkout switched branches mid-session (fix/partner-migration-data-gaps) — use `git worktree add` for cross-branch work; NEVER `npx twenty` outside a dir with real node_modules (npx silently fetches npm's unrelated "twenty" package); symlink node_modules into the worktree and call node_modules/.bin/twenty with explicit appPath.
- Diagnosis traps: /s/ 404s were app-level responses (flow-bridge "no company id"; GET-probe on POST route), not routing failures. Upstream added LOGIC_FUNCTION_LEGACY_ROUTE_CUTOFF (isolated *.withtwenty.com domains) — not yet active for our workspace, but /s/ deprecation is coming: watch it.
