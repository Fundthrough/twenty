...
// PROSPECT is the honest starting point: a lead is a prospect until they actually sign up on the
// FundThrough platform, which is exactly when their company gains a PRO company id
// (Client__c.PRO_Company_ID__c -> company.companyId). The importer and
// backfill-lead-status-from-signup.mjs promote PROSPECT to NEW_SIGN_UP on that signal alone, so
// the status means something rather than being stamped on every record that appears.
