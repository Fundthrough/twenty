...
describe('App installation', () => {
  it('should find the installed app in the applications list', async () => {
    ...
    expect(app?.name).toBe('HubSpot Partner');
  });
});

describe('Custom objects', () => {
  it('should have the HsPartnerDeal object installed', async () => {
    const client = new MetadataApiClient();
    ...
    const dealObj = result.objects.edges.find(
      (e: { node: { nameSingular: string } }) =>
        e.node.nameSingular === 'hsPartnerDeal',
    );
